...
首先按照继承的顺序进行基类的初始化 --> 然后按照成员变量的申明顺序初始化，和**构造函数初始化列表**中的顺序无关 --> 然后进行构造函数体的执行
...
1. 运算符重载：当需要重载`<<`或者`>>`的时候，由于左操作数是`ostream` 或`istream`
2. 跨类访问私有成员
3. 工具函数需要访问私有数据
...
2. **友元关系不可传递** 
   若类 `A` 是类 `B` 的友元，类 `B` 是类 `C` 的友元，类 `A` **不会自动成为** 类 `C` 的友元。 
3. **友元声明的位置** 
   友元函数的声明必须出现在类的内部（通常在 `public` 或 `private` 区域，但权限不影响友元的访问能力）。 
4. **友元函数不是成员函数**    
...
| 内部 `private` 类 | ❌ 无法访问             | 数据封装，防止误操作   |
| `PIMPL` 设计模式  | ❌ 无法访问             | API 设计，隐藏实现细节 |
| `friend` 友元类   | ✅ 友元可访问           | 需要控制访问权限       |

如果目标是让**类的普通成员函数无法访问数据成员**，最佳方案是**使用 `static` 方法** 或 **隐藏数据（`PIMPL`、内部类）**。
...
在继承中，父类的所有属性和操作都会被继承到子类中，但是由于继承的默认是私有继承，导致无法直接访问父类的方法。（在下面的代码中，如果是`class Test : Derived`那么Test的实例t无法直接调用`t.Derived::foo()`）。而在类中的`private`,`protected`,`public`修饰的操作或者属性，仅仅用于类的不同访问者的访问权限。
